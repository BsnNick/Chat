  ...
  implements Listener
{
  ...
  public void onChangeWorld(PlayerTeleportEvent e)
  {
    ...
    this.pl.permissions.remove(e.getPlayer());
    this.pl.permissions.put(e.getPlayer(), new Perm(e.getPlayer()));
  }
...
@EventHandler(priority=EventPriority.LOWEST)
  ...
  public void onVanish(PlayerCommandPreprocessEvent e) {
    if (e.isCancelled())
      return;
    if ((!e.getMessage().contains("/vanish")) && (!e.getMessage().equals("/v")))
      return;
    if (e.isAsynchronous()) {//Don't expect this to ever happen, but it's important
      final PlayerCommandPreprocessEvent ev = new PlayerCommandPreprocessEvent(e.getPlayer(), e.getMessage());
